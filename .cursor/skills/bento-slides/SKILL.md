...
name: bento-slides
description: >-
  Use when the user asks for a presentation, slide deck, слайды, презентацию,
  колоду, PowerPoint, .pptx rebuild, Bento, /bento-slides, or to create/edit
  a .bento.html file.
---
...
Read the local schema first: `.cursor/skills/bento-slides/agents.md`
(copy of https://bento.page/agents.md). Then follow the rest of this skill.

- Save new decks as `presentations/<Topic>.bento.html` (topic in the filename,
  spaces → underscores). Edit an existing file in that folder when the user
  points at one.
- Fetch the latest shell only when no `.bento.html` exists yet:
  `curl -fsSL https://bento.page/releases/slides/Bento_Slides.bento.html -o presentations/<Topic>.bento.html`
- Edit **only** the `#bento-doc` JSON block. Never regenerate the HTML runtime.
- Open the finished file in a browser and look at every slide before reporting done.
- Prefer charts, tables, morph, and state slides over bullet walls.
